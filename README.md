...
- **2026-05-03** — Person silhouette gets real 3D shape via point cloud. New `PointCloud` payload (`headset/scene.py`) — N points + color + billboard size, rendered as `THREE.Points` with `PointsMaterial` (sizeAttenuation, transparent, depthWrite off). Capture publisher now sends both: a flat polyline outline (no fill, kills the fan-triangulation artifact) plus the existing voxel-downsampled person cloud (`pts_vox`) as a PointCloud at 4 cm billboard size. From the camera side: real 3D shape with curvature. From the side: dot cloud follows the person's depth profile. From behind: still flat (single camera can't see through people — fundamental). Also: `mask_to_silhouettes_3d` now flattens the outline to per-contour median depth + applies `cv2.approxPolyDP` smoothing (~0.5% perimeter), so the polyline boundary is one clean closed loop instead of pixel-staircase.
- **2026-05-03** — Drone pose wired into the segmentation publisher. New [capture/drone_pose.py](capture/drone_pose.py) — `DronePoseToWorld` spawns `DronePoseReader` in a background thread, builds `T_NED_body` from the latest LOCAL_POSITION_NED + ATTITUDE_QUATERNION, and exposes a zero-able `T_world_camera` per the chain `R_axisswap @ inv(T_NED_drone0) @ T_NED_drone_now @ T_drone_camera`. World axes = drone-body-at-zero axis-swapped to WebXR convention; soldier holds the drone aimed forward at zero, world "forward" = soldier's forward. New `--pose-source drone --drone-port /dev/ttyUSB0` flags in `capture/run_segment.py`; auto-zeroes on first pose by default, press 'z' to re-zero. Drift caveat: outdoor GPS-EKF gives ~10–50 cm stationary drift, ~1–2 m/min under motion; indoor needs VIO or external pose. Camera mount default: forward-facing at drone centre.
- **2026-05-03** — Drone telemetry reader landed. New [drone/pose_reader.py](drone/pose_reader.py) — pymavlink-based `DronePoseReader` that subscribes a SiK-radio MAVLink link to LOCAL_POSITION_NED, ATTITUDE(_QUATERNION), GLOBAL_POSITION_INT, GPS_RAW_INT (and ODOMETRY if firmware emits it). CLI prints live pose; library API exposes `DronePoseSample` via callback for downstream (wire protocol, AR overlay) consumers. Supports direct serial (`/dev/ttyUSB0`) or UDP-from-MAVProxy (`udpin:127.0.0.1:14551`) so QGC can run alongside. Frame note: emits autopilot NED frame; conversion to ArUco-anchored world frame (calibrated `T_W_NED`) is deliberately deferred — silently assuming identity would misalign the AR overlay. `pymavlink` + `pyserial` added to requirements.
- **2026-05-03** — World-locked silhouettes via Quest-side pose. New `frame: "world" | "camera"` field on Cube + Polyline. With `--camera-on-headset`, publisher emits silhouettes in camera-local frame (axis swap only) tagged `frame="camera"`; renderer (`headset/web/index.html`) maintains two roots — `worldRoot` (identity) and `headRoot` (matrix snapshotted from `renderer.xr.getCamera().matrixWorld` each scene message). Camera-frame items get pinned to where the headset was when the scene arrived, so silhouettes stay world-locked when the user turns their head. `--camera-height` flag removed (Quest provides height via head pose). Known lag: head motion between capture and message-receive (~30–80 ms) shows as a small offset; full fix needs timestamped pose history.
- **2026-05-03** — Person wireframe is now the silhouette, not a cuboid. `mask_to_silhouettes_3d` traces the 2D mask contour (cv2.findContours), decimates to 80 pts, back-projects each pixel through depth → 3D polyline that follows the person's true curvature. New `Polyline` payload added to [headset/scene.py](headset/scene.py); renderer in [headset/web/index.html](headset/web/index.html) draws a closed `LineLoop` plus a fan-triangulated semi-transparent green fill (alpha 0.55). Run_segment now POSTs polylines instead of cubes; right pane in cv2 draws the silhouette outline in yellow.
- **2026-05-03** — Camera-on-headset transform. `--camera-on-headset` flag in [capture/run_segment.py](capture/run_segment.py) applies a static 4x4 mapping cam-optical (X right, Y down, Z forward) → WebXR (X right, Y up, -Z forward), plus a Y lift = `--camera-height` (default 1.55 m) so cubes don't render below the floor. Assumes the user is stationary at WebXR origin facing -Z; head-rotation lag is the obvious follow-up (needs headset pose flowing back to the publisher).
- **2026-05-03** — Live person wireframe → headset. [capture/person_wireframe.py](capture/person_wireframe.py) extracts an OBB per person cluster (DBSCAN ε=0.10 m) from the segmented cloud. [capture/run_segment.py](capture/run_segment.py) gains `--vr` (POST OBBs to `https://<host>:8443/scene` at 10 Hz) and `--transform PATH` (4x4 T_world_camera; identity for now since VR frame == camera frame, drone telemetry is the future swap). [headset/server.py](headset/server.py) accepts `POST /scene` and rebroadcasts to all WebXR clients. Right pane of the OpenCV preview now draws the projected 3D OBB in yellow so you can eyeball box quality before pushing.
- **2026-05-03** — Second capture pipeline: [capture/run_segment.py](capture/run_segment.py). Standalone (no ROS / no RTAB-Map) — RealSense → YOLOv8n-seg (`person` class only) → masked depth → back-projected point cloud (camera frame) → side-by-side OpenCV viewer (left: RGB+green mask, right: person pixels only) → `recordings/person_TIMESTAMP.ply` on exit. Picked YOLOv8-seg over YOLO-World+MobileSAM: ~50 ms vs ~250 ms CPU, single model, COCO `person` class is well-trained, mask quality difference is invisible after voxelization to 2 cm. Open-vocab segmentation is the future swap point if/when targets leave the COCO 80.
